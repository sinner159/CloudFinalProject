...
def getFullMatch( msg ):
    ...
    dl_src = mac.haddr_to_bin(eth.src)
    dl_dst = mac.haddr_to_bin(eth.dst)
    ...
    ip = pkt.get_protocol(ipv4.ipv4)
    if ip is not None :
        nw_src = ipv4_to_int(ip.src)
        nw_dst=ipv4_to_int(ip.dst) 
        nw_proto = ip.proto
        nw_tos = ip.tos
    ...
    a = pkt.get_protocol(arp.arp)
    if a is not None :
        nw_src = ipv4_to_int(a.src_ip)
        nw_dst = ipv4_to_int(a.dst_ip)
        nw_proto = a.opcode
...
def getReverseMatch( msg ):
    ...
    match = parser.OFPMatch( 
    #in_port = in_port,
    #dl_src=mac.haddr_to_bin(dl_dst), 
    #dl_dst=mac.haddr_to_bin(dl_src), 
    #dl_vlan=dl_vlan, 
    #dl_vlan_pcp=dl_vlan_pcp, 
    dl_type=dl_type, 
    nw_tos=nw_tos, 
    nw_proto=nw_proto, 
    nw_src=ipv4_to_int(nw_dst), 
    nw_dst=ipv4_to_int(nw_src), 
    tp_src=tp_dst, 
    tp_dst=tp_src )
    return match
...
def subnetMatch(subnet, address):
    net = subnet.split('/')[0]
    bits = int(subnet.split('/')[1])
    mask = ~((1 << 32 - bits) - 1)
    print (net, bits, mask)
    addrnet = int(address) & mask
    return addrnet ^ net == 0
